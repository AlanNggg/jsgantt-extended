...
export const parseXMLString = function (pStr, pGanttVar) {
    ...
    if (typeof (<any>window).DOMParser != "undefined") {
        xmlDoc = new (<any>window).DOMParser().parseFromString(
            pStr,
            "text/xml"
        );
    } else if (
...
export const AddXMLTask = function (pGanttVar, pXmlDoc) {
    ...
    if (project == "http://schemas.microsoft.com/project") {
        ...
        for (i = 0; i < n; i++) {
            ...
            let vOutlineNumber;
            if (uid != 0)
                vOutlineNumber = getXMLNodeValue(
                    Task[i],
                    "OutlineNumber",
                    2,
                    "0"
        ...
        for (i = 0; i < n; i++) {
            ...
            if (pID != 0) {
                ...
                let pRemovable = getXMLNodeValue(Task[i], "Removable", 1, 0);

                let pParent = 0;

                let vOutlineLevel = getXMLNodeValue(
                    Task[i],
                    "OutlineLevel",
                    1,
                    0
                ...
                for (k = 0; k < j; k++) {
                    let depUID = getXMLNodeValue(
                        predecessors[k],
                        "PredecessorUID",
                        1,
                        -1
                    );
                    let depType = getXMLNodeValue(
                        predecessors[k],
                        "Type",
                        1,
                        1
        ...
        for (i = 0; i < n; i++) {
            ...
            if (pID != 0) {
                ...
                let pRemovable = getXMLNodeValue(Task[i], "pRemovable", 1, 0);
