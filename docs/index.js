...
function start(e) {
    ...
    if (g.getDivId() != null) {
        ...
        const vEditable = document.querySelector("#editable:checked")
            ? true
            : false;
        const vDraggable = document.querySelector("#draggable:checked")
            ? true
            : false;
        const vUseSort = document.querySelector("#sort:checked") ? true : false;
        const newtooltiptemplate = document.getElementById("tooltiptemplate")
            .value
            ? document.getElementById("tooltiptemplate").value
            : null;
        let vColumnOrder;
        if (document.querySelector("#vColumnOrder").value) {
            vColumnOrder = document
                .querySelector("#vColumnOrder")
                .value.split(",");
        }
        ...
        vShowStartDate = document.querySelector("#vShowStartDate:checked")
            ? 1
            : 0;
        vShowEndDate = document.querySelector("#vShowEndDate:checked") ? 1 : 0;
        vShowPlanStartDate = document.querySelector(
            "#vShowPlanStartDate:checked"
        )
            ? 1
            : 0;
        ...
        vShowEndWeekDate = document.querySelector("#vShowEndWeekDate:checked")
            ...
            : 0;
        vTotalHeight =
            document.querySelector("#vTotalHeight").value || undefined;

        vShowWeekends = document.querySelector("#vShowWeekends:checked")
            ? 1
            : 0;
...
function generateTooltip(task) {
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            console.log(
                `Generated dynamic async template for '${task.getName()}'`
            );
            resolve(
                `Tooltip content from the promise after ${Math.round(delay)}ms`
            );
        }, delay);
    });
}
